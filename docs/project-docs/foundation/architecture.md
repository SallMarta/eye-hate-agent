...
- **Adapters:** Agent-specific formatters (Claude, Copilot, Antigravity, Gemini CLI, Hermes, OpenCode) that mutate the raw template before saving it to the target platform instruction surface (e.g., `.agents/`).
...
graph TD
    ...
    F --> G[Write to .agents/ or .claude/ or .github/ or .gemini/ or .hermes/ or .opencode/]
