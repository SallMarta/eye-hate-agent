...
- reusable skills and prompt modules
...
AI customization repos become difficult to trust when rules, skills, prompts, and project docs each contain their own version of the truth.

That drift usually shows up as:

- stale project names
- hardcoded stack commands in the wrong layer
- prompt outputs that no longer match the expected document contract
...
- keep prompts responsible for creating and refreshing project docs
...
- using prompt text or skill text as the durable source of project truth
...
| Maintainability | Rule, skill, and prompt updates can be made without rewriting project-specific examples |
