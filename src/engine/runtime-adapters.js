...
function buildGeminiRuleFile() {
  return `---
description: "EHA agent rules"
---

${loadRuleContent('gemini')}`;
}
...
const RUNTIME_ADAPTERS = {
  ...
  gemini: {
    id: 'gemini',
    name: 'Gemini',
    description: 'Generates .gemini/commands/eha/ command files, .gemini/skills/ and .gemini/rules/',
    generateFiles(rootDir, workflows, skills) {
      const files = [];
      for (const workflow of workflows) {
        files.push({
          relativePath: path.join('.gemini', 'commands', 'eha', `eha-${workflow.commandName}.md`),
          content: buildGeminiCommandFile(workflow),
        });
      }
      files.push({
        relativePath: path.join('.gemini', 'commands', 'eha', 'README.md'),
        content: `# EHA Gemini commands\n\nGenerated by \`eha init\`. Use \`/eha-bootstrap\`, \`/eha-refresh\`, \`/eha-parity\`, or \`/eha-discuss\`.\n`,
      });
      
      for (const skill of skills) {
        files.push({
          relativePath: path.join('.gemini', 'skills', `eha-${skill.commandName}.md`),
          content: buildGeminiSkillFile(skill),
        });
      }
      
      files.push({
        relativePath: path.join('.gemini', 'rules', 'eha-agent-rules.md'),
        content: buildGeminiRuleFile(),
      });
