...
- **Prefix & Lookback Integrity (Claude):** Cache reads cost 90% less but require a byte-identical prefix in tools → system → messages order. Never reorder, add, or remove tool definitions mid-session. Never inject dynamic content (timestamps, per-request IDs) into the system prompt — move it to a late user message instead. In heavy agentic loops, be aware of the ~20-block lookback window: if a single turn produces more than ~15 tool-use/tool-result pairs, place intermediate cache breakpoints to prevent silent cache misses on the next turn.
