...
- **2.4** **Session Continuity (No Dynamic Compaction):** Never modify, compact, or delete prior chat turns within the active session, as this destroys the hardware prefix cache. If the context window is getting too heavy (roughly 65% capacity), write the progress, decisions, and open threads to `/memories/session/` in the active repository (and ensure `/memories/session/` is added to `.gitignore` if created). Advise the user to start a fresh chat session with that memory file loaded.
